...
class LIBLEIDENALG_EXPORT MutableVertexPartition
{
  public:
    ...
    virtual MutableVertexPartition* create(Graph* graph) = 0;
    virtual MutableVertexPartition* create(Graph* graph, vector<size_t> const& membership) = 0;

    inline size_t membership(size_t v) { return this->_membership[v]; };
    inline vector<size_t> const& get_membership() const { return this->_membership; };
    ...
    vector<size_t> get_neigh_comms(size_t comm);
    ...
    virtual double quality(double resolution_parameter) { return this->quality(); };

    virtual MutableVertexPartition* clone() = 0;

    void merge_communities(size_t comm_from, size_t comm_to);
    double diff_move_community(size_t comm_from, size_t comm_to);
